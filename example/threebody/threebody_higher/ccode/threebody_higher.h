...
#define tsm_pow_c(a,b,c,k);\
	mpfr_pow(c,a,b,GMP_RNDN)

#define tsm_pow(A,b,C,k);\
	mpfr_set_str(_son,"0.0",10,GMP_RNDN);\
	mpfr_set_str(_father,"0.0",10,GMP_RNDN);\
	mpfr_set_str(_zero,"0.0",10,GMP_RNDN);\
	if (k==0)\
	{\
		mpfr_pow(_father,A[0],b,GMP_RNDN);\
	}\
	else\
	{\
		if(!mpfr_equal_p(A[0],_zero))\
		{\
			if (_id==0)\
			{\
				mpfr_mul_si(_father,b,k,GMP_RNDN);\
				mpfr_mul(_father,_father,C[0],GMP_RNDN);\
				mpfr_mul(_father,_father,A[k],GMP_RNDN);\
			}\
			for(_i=_id+1;_i<k;_i+=_size)\
			{\
				mpfr_mul_si(_son,b,k-_i,GMP_RNDN);\
			...
			mpfr_mul_si(_son,A[0],k,GMP_RNDN);\
			mpfr_div(_father,_father,_son,GMP_RNDN);\
		}\
		else\
		{\
			mpfr_set(_father,_zero,GMP_RNDN);\
		}\
	}\
