...
int main(int argc, char *argv[]){
	tsm_init();
	tsm_clock_start();
	FILE *fp=fopen("../ans/result_lorenz96.txt","w");
	int i, j, order;
	order = _order;
	mpfr_t x_0[order+1], x_1[order+1], x_2[order+1], x_3[order+1], x_4[order+1];
	mpfr_t saver_0[order+1], saver_1[order+1], saver_2[order+1], saver_3[order+1], saver_4[order+1], saver_5[order+1], saver_6[order+1], saver_7[order+1], saver_8[order+1], saver_9[order+1], saver_10[order+1], saver_11[order+1], saver_12[order+1], saver_13[order+1], saver_14[order+1], saver_15[order+1], saver_16[order+1], saver_17[order+1], saver_18[order+1], saver_19[order+1], saver_20[order+1], saver_21[order+1], saver_22[order+1], saver_23[order+1], saver_24[order+1];
	mpfr_t constant_0, constant_1;
	mpfr_t t[order+1];
	mpfr_t inc_time, final_time;
	mpfr_t x_0_p, x_1_p, x_2_p, x_3_p, x_4_p;
	mpfr_inits2(_prec, inc_time, final_time, (mpfr_ptr) 0);
	mpfr_inits2(_prec, x_0_p, x_1_p, x_2_p, x_3_p, x_4_p, constant_0, constant_1, (mpfr_ptr) 0);
	for(i=0; i<order+1; i++){
		mpfr_inits2(_prec, t[i], x_0[i], x_1[i], x_2[i], x_3[i], x_4[i], saver_0[i], saver_1[i], saver_2[i], saver_3[i], saver_4[i], saver_5[i], saver_6[i], saver_7[i], saver_8[i], saver_9[i], saver_10[i], saver_11[i], saver_12[i], saver_13[i], saver_14[i], saver_15[i], saver_16[i], saver_17[i], saver_18[i], saver_19[i], saver_20[i], saver_21[i], saver_22[i], saver_23[i], saver_24[i], (mpfr_ptr) 0);
	}
	...
	mpfr_set_str(constant_0, "-1.", 10, GMP_RNDN);
	mpfr_set_str(constant_1, "8.", 10, GMP_RNDN);
	mpfr_set_str(t[0], "0", 10, GMP_RNDN);
	mpfr_set_str(t[1], "1.0", 10, GMP_RNDN);
	mpfr_set_str(x_0_p, "8.01", 10, GMP_RNDN);
	mpfr_set_str(x_1_p, "8", 10, GMP_RNDN);
	mpfr_set_str(x_2_p, "8", 10, GMP_RNDN);
	mpfr_set_str(x_3_p, "8", 10, GMP_RNDN);
	mpfr_set_str(x_4_p, "8", 10, GMP_RNDN);
	int fprint_flag=1;
	...
	int PRE=_prec;
	double delta_T=50, remaining_T=500, gamma=1.15, lyap=0.48;
	int m=0;
	...
	while (mpfr_less_p(t[0], final_time)){
		if (fprint_flag && m%(int)(delta_T/print_time_d)==0 && m<=(int)((T_c-remaining_T)/print_time_d)){
			PRE=(int)ceil(gamma*lyap*(T_c-T_s)/log(10.0));
				order=(int)ceil(-0.0007627267239713758*PRE*PRE+1.652700926634416*PRE+16.32741271741077);
		...
		for (i = 0; i < order; i++){
			tsm_mul_cs(constant_0,x_0,saver_0,i);
			tsm_mul_cs(constant_0,x_3,saver_1,i);
			tsm_add_ss(x_1,saver_1,saver_2,i);
			tsm_mul_ss(x_4,saver_2,saver_3,i);
			tsm_add_cs(constant_1,saver_0,saver_4,i);
			tsm_add_ss(saver_4,saver_3,saver_5,i);
			tsm_mul_cs(constant_0,x_1,saver_6,i);
			tsm_mul_cs(constant_0,x_4,saver_7,i);
			tsm_add_ss(x_2,saver_7,saver_8,i);
			tsm_mul_ss(x_0,saver_8,saver_9,i);
			tsm_add_cs(constant_1,saver_6,saver_10,i);
			tsm_add_ss(saver_10,saver_9,saver_11,i);
			tsm_mul_cs(constant_0,x_2,saver_12,i);
			tsm_add_ss(x_3,saver_0,saver_13,i);
			tsm_mul_ss(x_1,saver_13,saver_14,i);
			tsm_add_cs(constant_1,saver_12,saver_15,i);
			tsm_add_ss(saver_15,saver_14,saver_16,i);
			tsm_add_ss(x_4,saver_6,saver_17,i);
			tsm_mul_ss(x_2,saver_17,saver_18,i);
			tsm_add_cs(constant_1,saver_1,saver_19,i);
			tsm_add_ss(saver_19,saver_18,saver_20,i);
			tsm_add_ss(x_0,saver_12,saver_21,i);
			tsm_mul_ss(x_3,saver_21,saver_22,i);
			tsm_add_cs(constant_1,saver_7,saver_23,i);
			tsm_add_ss(saver_23,saver_22,saver_24,i);
			mpfr_div_ui(x_0[i+1],saver_5[i],(i+1),GMP_RNDN);
			mpfr_div_ui(x_1[i+1],saver_11[i],(i+1),GMP_RNDN);
			mpfr_div_ui(x_2[i+1],saver_16[i],(i+1),GMP_RNDN);
			mpfr_div_ui(x_3[i+1],saver_20[i],(i+1),GMP_RNDN);
			mpfr_div_ui(x_4[i+1],saver_24[i],(i+1),GMP_RNDN);
		}
		...
		mpfr_fprintf(fp,"%1.500Re\t",x_2_p);
		mpfr_fprintf(fp,"%1.500Re\t",x_3_p);
